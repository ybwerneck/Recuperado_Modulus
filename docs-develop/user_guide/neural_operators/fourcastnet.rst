...
Given an inital condition from the ERA5 dataset as input, FourCastNet recursively applies an Adaptive Fourier Neural Operator (AFNO) network to predict their dynamics at later time steps.
...
.. literalinclude:: /external/examples/fourcastnet/conf/config_FCN.yaml
   :language: yaml
...
.. literalinclude:: /external/examples/fourcastnet/src/dataset.py
   :language: python
   :lines: 11-
...
.. literalinclude:: /external/examples/fourcastnet/fcn_era5.py
   :language: python
   :lines: 22-39
...
.. literalinclude:: /external/examples/fourcastnet/src/fourcastnet.py
   :language: python
   :lines: 12-
...
Below the ACC and RSME values of the model trained in Modulus is compared to the results of the original work with excellent comparison to the original work [#pathak2022fourcastnet]_.
...
.. figure:: /images/user_guide/fourcastnet_rmse.png
   :alt: Modulus FourCastNet RSME
   :width: 90.0%
   :align: center

   Comparison of the predictive root mean square error (RSME) of each variable between the original FourCastNet model (Original) and the version trained in Modulus.
