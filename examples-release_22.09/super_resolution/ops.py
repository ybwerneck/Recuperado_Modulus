...
class FlowOps(torch.nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.register_buffer("ddx1D", torch.Tensor(
    ...
    def ddx(self, inpt, dx, channel, dim, padding_mode="replicate"):
        ...
        ddx3D = torch.reshape(self.ddx1D, shape=[1, 1] + dim * [1] + [-1] + (2 - dim) * [1])
        padding = _triple(3) + _triple(3)

        output = F.conv3d(
            F.pad(var, padding, mode=padding_mode), ddx3D, stride=1, padding=0, bias=None
        )
